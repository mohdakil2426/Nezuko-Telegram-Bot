...
async def test_mute_bot_account():
    ...
    context.bot.restrict_chat_member = AsyncMock(
        side_effect=BadRequest("Can't restrict bots")
    )
...
async def test_concurrent_verification_calls():
    ...
    with patch('bot.services.verification.cache_get', new_callable=AsyncMock) as mock_cache_get, \
        ...
        tasks = [
            check_membership(user_id, "@testchannel", context)
...
async def test_concurrent_mute_operations():
    ...
    tasks = [
        restrict_user(group_id, user_id, context)
        for user_id in range(100, 110)
    ]
...
async def test_cache_expired_entry():
    ...
    with patch('bot.core.cache._redis_available', True), \
...
async def test_unprotect_not_protected_group():
    ...
    from bot.database.crud import toggle_protection, get_protected_group
