 ...
 * Uses canvas for performance with mouse-reactive particles.
 * Respects theme settings for density and animations.
 */
export function ParticleBackground() {
  ...
  const { accentHex, particleDensity, particles, animations, reducedMotion } = useThemeConfig();
  ...
  const initParticles = useCallback(
    (width: number, height: number): Particle[] => {
      ...
      const particleCount = Math.min(
        Math.floor((width * height) / adjustedDensityFactor),
        Math.floor(80 * (particleDensity / 50))
      );
      ...
      const colors = isDark
        ? [accentHex, "#ffffff", `${accentHex}80`, `${accentHex}40`]
        : [accentHex, "#000000", `${accentHex}80`, `${accentHex}40`];
    ...
    [resolvedTheme, accentHex, particleDensity]
  );
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      particles.forEach((particle, i) => {
        ...
        particles.slice(i + 1).forEach((other) => {
          ...
          if (connDist < 120) {
            ...
            const alpha = (isDark ? 0.15 : 0.1) * (1 - connDist / 120);
            ctx.strokeStyle = `${accentHex}${Math.round(alpha * 255)
              .toString(16)
              .padStart(2, "0")}`;
            ctx.lineWidth = 0.5;
            ctx.stroke();
      ...
      const glowAlpha = isDark ? "14" : "0d"; // ~0.08 and ~0.05 in hex
      gradient.addColorStop(0, `${accentHex}${glowAlpha}`);
  ...
  }, [initParticles, resolvedTheme, animations, accentHex, reducedMotion]);
