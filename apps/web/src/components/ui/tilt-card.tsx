...
export function TiltCard({
  ...
  const effectiveGlowColor = glowColor || `${accentHex}25`;
  ...
  const handleMouseMove = useCallback(
    (e: React.MouseEvent<HTMLDivElement>) => {
      if (!cardRef.current || !enableTilt || reducedMotion) return;
    ...
    [enableTilt, intensity, reducedMotion]
  );
  ...
  // Disable effects if reduced motion is enabled
  ...
  return (
    <motion.div
      ref={cardRef}
      className={cn(
        "relative overflow-hidden rounded-2xl border border-[var(--nezuko-border)]",
