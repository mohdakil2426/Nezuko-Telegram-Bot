...
import { Search, Filter, Download, Terminal, Activity, Clock, User as UserIcon, AlertCircle, CheckCircle2, FileStack, AlertTriangle, Users } from 'lucide-react';
                            ...
                            <AnimatePresence mode="wait">
                                ...
                                >
                                    {filteredSystemLogs.length > 0 ? filteredSystemLogs.map((log) => (
                                        <tr key={log.id} className="border-b border-(--nezuko-border)/50 hover:bg-(--nezuko-surface-hover) transition-colors group">
                                            <td className="py-3.5 px-3 text-(--text-muted) w-36 whitespace-nowrap">{log.timestamp}</td>
                                            ...
                                            <td className="py-3.5 px-3 text-(--text-primary)">
                                                {log.message}
                                                {log.details && <div className="text-xs text-(--text-muted) mt-1">{log.details}</div>}
                                            </td>
                                        </tr>
                                    )) : (
                                        <tr>
                         ...
                         <table className="w-full text-left border-collapse">
                            ...
                            <AnimatePresence mode="wait">
                                <motion.tbody 
                                    ...
                                    className="text-sm"
                                >
                                     {filteredBotLogs.length > 0 ? filteredBotLogs.map((log) => (
                                        <tr key={log.id} className="border-b border-(--nezuko-border)/50 hover:bg-(--nezuko-surface-hover) transition-colors group">
                                            <td className="py-3.5 px-3">
                                                <div className="flex items-center gap-3">
                                                    <img src={log.userAvatar} alt={log.user} className="w-8 h-8 rounded-full bg-(--nezuko-surface)" />
                                            ...
                                            <td className="py-3.5 px-3">
                                                <span className={cn(
                                                    "font-mono text-xs",
                                                    log.latency < 100 ? "text-green-500" : log.latency < 300 ? "text-yellow-500" : "text-red-500"
                                                )}>
                                                    {log.latency}ms
                                                </span>
                                            </td>
